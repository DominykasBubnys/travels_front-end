...
const PlaceDetails = () => {
    ...
    const getUsersByIds = async(ids) => {
        ...
        try {
            for(let i = 0; i < ids.length; i++){
                const reactedUserRequest = await fetch(`${process.env.REACT_APP_BACKEND_URL}/user/${ids[i]}`);
                if(!reactedUserRequest.ok) throw new Error("Cannot fetch users who likes this place");
    ...
    useEffect(() => {
        const loadPlaces = async() => {
            ...
            try {
                ...
                const request = await fetch(`${process.env.REACT_APP_BACKEND_URL}/place/${pid}`);
                ...
                if(!reactedIds) throw new Error("Cannot fetch users ids who likes this place")
    ...
    return <React.Fragment>
        ...
        {!isloading && loadedPlace && <div className={classes.container}>
            ...
            <div className={classes.extra}>
                ...
                <div className={classes.reactionsContainer}>
                    ...
                    <Button onClick={changeReactionsHandler}>Likes: {loadedPlace.likedBy.length}</Button>

                    { showReactions && (reactedUsersList.length>0) && <div className={classes.reactions}>
                        {reactedUsersList.map(user => <button style={{margin: "10px auto", border: "none"}} onClick={linkToReactedUserProfile.bind(null,user._id)}>{user.name}</button>)}
                    </div>}
