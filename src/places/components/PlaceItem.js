...
const PlaceItem = (props) => {
    ...
    const confirmDeleteHandler = async() => {
        ...
        try {
            const sendRequest = await fetch(`${process.env.REACT_APP_BACKEND_URL}/delete-place/${props.id}`, {
                method: "DELETE",
                headers: {
    ...
    const reactionHandler = async() => {
        ...
        try {
            const sendRequest = await fetch(`${process.env.REACT_APP_BACKEND_URL}/liked-place/${props.id}`, {
                method: "POST",
                headers: {
                  "Content-Type": "application/json"
                },
    ...
    return (
        ...
        <Fragment>
            ...
            <li className="place-item-container">
                ...
                { !isloading && <Card className="place-item__content">
                    <div className="place-item">
                        ...
                        <div className="place-item__info">
                            <h1>{props.title}</h1>
                            <h2>{props.address}</h2>                            
                            <h3 style={{color: "white", marginBottom: "2rem"}}>{likesAmount} {likesAmount > 1? "visitors" : "visitor"} likes this place!</h3>
                    ...
                    <div className="place-item__actions">
                        <Button inverse onClick={openMapHandler} >SHOW COORDINATES</Button>
                        
                        {Auth.isLoggedIn && !reaction && <Button inverse onClick={reactionHandler} >LIKE [{likesAmount}]</Button>}
                        {Auth.isLoggedIn && reaction && <Button inverse onClick={reactionHandler} >DISLIKE [{likesAmount}]</Button>}
                        
                        {props.showControllers && <React.Fragment>
